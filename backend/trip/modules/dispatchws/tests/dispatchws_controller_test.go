...
func TestServeWS_DeniesWhenUnauthorized(t *testing.T) {
	server, sign, _ := newDispatchWSServer(t, false)
	defer server.Close()
...
func TestServeWS_StandbyThenFindDriver(t *testing.T) {
	server, sign, store := newDispatchWSServer(t, true)
	defer server.Close()
	...
	nearby, err := store.Nearby(context.Background(), -6.2088, 106.8456, 0.5, 10)
	require.NoError(t, err)
	require.Len(t, nearby, 1)
	assert.Equal(t, "drv-1", nearby[0].UserID)

	customer := sign("cst-1", "cst@example.com", "customer")
	req, err := http.NewRequest(http.MethodGet, server.URL+"/api/trip/dispatch/find-driver?current_location=-6.2088&current_location=106.8456", nil)
	require.NoError(t, err)
...
func TestServeWS_AcceptsQueryToken(t *testing.T) {
	server, sign, _ := newDispatchWSServer(t, true)
	defer server.Close()
...
func TestServeWS_SecondConnectReplacesFirst(t *testing.T) {
	...
	nearby, err := store.Nearby(context.Background(), -6.2090, 106.8458, 0.5, 10)
	require.NoError(t, err)
	require.Len(t, nearby, 1)
	assert.Equal(t, "drv-1", nearby[0].UserID)
}

func newDispatchWSServer(t *testing.T, allow bool) (*httptest.Server, func(userID, email, role string) string, *drivergeo.Store) {
	t.Helper()
	...
	return httptest.NewServer(router), sign, store
}
