...
import { isWeb, tva, withStyleContext } from '@gluestack-ui/utils/nativewind-utils';
...
// On native, Tailwind `flex` is not needed (View is already flex). Keep web `display:flex`.
const formControlStyle = tva({
  base: `${isWeb ? 'flex ' : ''}flex-col w-full`,
});
...
const formControlErrorStyle = tva({
  base: `${isWeb ? 'flex ' : ''}flex-row justify-start items-center mt-1 gap-1`,
});
...
const formControlHelperStyle = tva({
  base: `${isWeb ? 'flex ' : ''}flex-row justify-start items-center mt-1 font-body`,
});
...
const formControlLabelStyle = tva({
  base: `${isWeb ? 'flex ' : ''}flex-row justify-start items-center mb-1`,
});
